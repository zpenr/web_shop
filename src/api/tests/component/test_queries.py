...
def sample_data(session: Session):
    ...
    emp1 = Employee(
        name="Boss", surname="Bossov", login="boss", password="hash", id_job=job.id
    )
    session.add(emp1)
    session.flush()
    ...
    rcp = Receipt(created_at=datetime.now(), id_employee=emp1.id)
    session.add(rcp)
    session.flush()

    sale = Sale(id_receipt=rcp.id, id_product=prod1.id, quantity=10)
    session.add(sale)
    session.flush()
...
def test_insert_sale_with_storage_check_success(session, sample_data):
    ...
    session.flush()
    result = Queries.insert_sale_with_storage_check(
        sample_data["prod1"].id, 5, 1, session
    )
    session.flush()
...
def test_insert_sale_insufficient_quantity(session, sample_data):
    with pytest.raises(exceptions.NotEnoughProductException):
        Queries.create_receipt(datetime.now(), sample_data["emp2"].id, session)
        session.flush()
        Queries.insert_sale_with_storage_check(sample_data["prod2"].id, 20, 1, session)
...
def test_employee_by_login(session, sample_data):
    employee = Queries.employee_by_login(
        login=sample_data["emp2"].login, session=session
    )
...
@pytest.mark.parametrize(
    "id, price, quantity_at_storage", [(1, None, 100), (1, 10, 90), (2, 15, None)]
)
def test_update_product(id, price, quantity_at_storage, session, sample_data):
    ...
    assert (
        upd_product.quantity_at_storage == quantity_at_storage
...
@pytest.mark.parametrize(
    "min_sum, max_sum, min_date, max_date, product_id, employee_id",
    [
...
def test_filtered_sales(
    min_sum, max_sum, min_date, max_date, product_id, employee_id, session, sample_data
):
    sales = Queries.filtered_sales(
    ...
    for sale in sales:
        ...
        assert (
            sale.receipt.id_employee == employee_id if employee_id is not None else True
        )
...
@pytest.mark.parametrize(
    "make_sales, add_categories, add_products, redact_products, add_jobs, add_boss",
    [(True, True, False, True, True, False)],
)
def test_insert_permission(
    make_sales,
    add_categories,
    add_products,
    redact_products,
...
@pytest.mark.parametrize(
    "name, price, id_category, quantity_at_storage", [("name", 100, 1, 10)]
)
def test_insert_product(
    name, price, id_category, quantity_at_storage, session, sample_data
):
    product = Queries.insert_product(
        name, price, id_category, quantity_at_storage, session
    )
...
@pytest.mark.parametrize(
    "name, surname, login, password, id_job",
...
def test_delete_product_nonexistent_raises_not_found(session, sample_data):
    with pytest.raises(exceptions.NotFoundError) as exc_info:
        Queries.delete_product(9999, session)
...
def test_update_product_nonexistent_raises_attribute_error(session, sample_data):
    with pytest.raises(AttributeError):
        Queries.update_product(9999, 100, 10, session)


def test_filtered_products_empty_result(session, sample_data):
    products = Queries.filtered_products(999, 0, 1000, session)
    assert products == []


def test_filtered_products_invalid_range(session, sample_data):
    products = Queries.filtered_products(None, 200, 100, session)
    assert products == []


def test_filtered_sales_empty_result(session, sample_data):
    sales = Queries.filtered_sales(session, min_sum=100000, max_sum=200000)
    assert sales == []


def test_filtered_sales_by_nonexistent_product(session, sample_data):
    sales = Queries.filtered_sales(session, product_id=9999)
    assert sales == []


def test_get_product_by_id_nonexistent_raises(session, sample_data):
    with pytest.raises(Exception):
        Queries.get_product_by_id(9999, session)


def test_insert_product_negative_price_raises(session, sample_data):
    with pytest.raises(ValueError, match="Price must be greater or equal than zero"):
        Queries.insert_product("Bad", -10, sample_data["cat"].id, 5, session)


def test_insert_product_negative_quantity_allowed(session, sample_data):
    product = Queries.insert_product(
        "NegativeStock", 100, sample_data["cat"].id, -3, session
    )
    assert product.quantity_at_storage == -3


def test_insert_employee_duplicate_login_raises(session, sample_data):
    job = sample_data["job"]
    with pytest.raises(IntegrityError):
        Queries.insert_employee(
            "Clone", "Clone", sample_data["emp2"].login, "hash", job.id, session
        )
        session.flush()


def test_add_boss_nonexistent_employee(session, sample_data):
    with pytest.raises(AttributeError):
        Queries.add_boss(9999, 1, session)


def test_products_to_buy_zero_threshold(session, sample_data):
    prod = sample_data["prod2"]
    prod.quantity_at_storage = 0
    session.flush()
    products = Queries.products_to_buy(0, session)
    assert any(p.id == prod.id for p in products)


def test_create_receipt_with_future_date(session, sample_data):
    future = datetime(2030, 1, 1)
    receipt = Queries.create_receipt(future, sample_data["emp1"].id, session)
    assert receipt.created_at == future
