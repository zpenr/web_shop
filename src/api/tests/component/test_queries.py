...
def sample_data(session: Session):
    cat = Categories(name="Food")
    session.add(cat)
    session.flush()

    prod1 = Products(name="Bread", price=50, id_category=cat.id, quantity_at_storage=100)
    prod2 = Products(name="Milk", price=80, id_category=cat.id, quantity_at_storage=10)
    session.add_all([prod1, prod2])
    session.flush()

    root = Roots(make_sales=True, add_categories=True, add_products=True,
                 redact_products=True, add_jobs=True, add_boss=True)
    session.add(root)
    session.flush()

    job = Jobs(name="Seller", root_id=root.id)
    session.add(job)
    session.flush()

    emp1 = Employees(name="Boss", surname="Bossov", login="boss", password="hash", id_job=job.id)
    session.add(emp1)
    session.flush()

    emp2 = Employees(name="Ivan", surname="Ivanov", login="ivan", password="hash", id_job=job.id, boss=emp1.id)
    session.add(emp2)
    session.flush()

    rcp = Receipts(created_at=datetime.now(), id_employee=1)
    session.add(rcp)
    session.flush()

    sales = Sales(id_receipt=1, id_product=1,quintity=10)
    session.add(sales)
    session.flush()

    return {"cat": cat, "prod1": prod1, "prod2": prod2, "emp1": emp1, "emp2":emp2, "job": job, "root": root, "rcp":rcp, "sales":sales}
...
def test_insert_sale_with_storage_check_success(session, sample_data):
    Queries.create_rececipt(datetime.now(), sample_data["emp2"].id, session)
    session.flush()
    ...
    prod = session.get(Products, sample_data["prod1"].id)
    assert prod.quantity_at_storage == 95

def test_insert_sale_insufficient_quantity(session, sample_data):
    with pytest.raises(exceptions.NotEnougthProductException):
        Queries.create_rececipt(datetime.now(), sample_data["emp2"].id, session)
        session.flush()
        result = Queries.insert_sale_with_storage_check(sample_data["prod2"].id, 20, 1, session) #noqa
...
def test_all_employess(session, sample_data):
...
def test_filtered_sales(min_sum, max_sum, min_date, max_date, product_id, employee_id, session, sample_data):
    ...
    for sale in sales:
        s = sale.quintity * sale.product.price
...
def test_insert_root(make_sales, add_categories, 
                    add_products, redact_products, 
                    add_jobs, add_boss,
                    session, sample_data):
    
    root = Queries.insert_root(make_sales, add_categories, 
                               add_products, redact_products, 
                               add_jobs, add_boss, session)
    
    assert root.make_sales == make_sales
    assert root.add_categories == add_categories
    assert root.add_products == add_products
    assert root.redact_products == redact_products
    assert root.add_jobs == add_jobs
    assert root.add_boss == add_boss
    
@pytest.mark.parametrize("boss_id",[
    (1)
    ])
...
@pytest.mark.parametrize("id, boss_id",[
    (2, 1)
    ])
...
@pytest.mark.parametrize("name, root_id",[
    ("name", 1)
])
def test_insert_job(name, root_id, session, sample_data):
    job = Queries.insert_job(name, root_id, session)
...
@pytest.mark.parametrize("red_quantity", [
    (50)
])
